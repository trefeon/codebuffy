...
| `src/routes/` | `openai.ts`, `anthropic.ts`, `responses.ts`, `metrics.ts` |
...
| `src/credentials/` | Store + refresh + login (`store.ts`, `crypto.ts`, `refresh.ts`, `device-flow.ts`, `file-importer.ts`, `watcher.ts`, `types.ts`) |
